...
export default function GamificationWidget({ session, isDev, refreshTrigger }) {
  ...
  return (
    <motion.div 
      ...
      className="bg-gradient-to-r from-primary-900 to-primary-700 rounded-xl shadow-md p-4 text-white mb-6 flex flex-col sm:flex-row items-center justify-between gap-4"
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex items-end gap-3">
            ...
            >
              {stats.hoy} <span className="text-base font-normal text-primary-200">hoy</span>
            </motion.h3>
      ...
      {stats.insignias.length > 0 && (
        <div className="flex flex-col sm:items-end">
          ...
          <div className="flex gap-2">
            {stats.insignias.map((logro, index) => (
              <motion.div 
                key={logro.id}
                initial={{ scale: 0, rotate: -180 }}
                animate={{ scale: 1, rotate: 0 }}
                transition={{ type: 'spring', delay: index * 0.1, stiffness: 260, damping: 20 }}
                whileHover={{ scale: 1.15, rotate: 10 }}
                title={logro.nombre} 
                className="flex items-center justify-center w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 transition-colors border border-white/20 shadow-sm cursor-help relative group"
              >
